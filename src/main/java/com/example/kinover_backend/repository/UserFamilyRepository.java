...
public interface UserFamilyRepository extends JpaRepository<UserFamily, UUID> {
    ...
    /**
     * ✅✅✅ 핵심: "가장 최근 생성된 가족"의 familyId 1개만 가져오기
     * - Family.createdAt 기준 desc
     * - Pageable로 1개 제한 (PageRequest.of(0,1))
     * - uf.family null 방어
     */
    @Query("""
        select uf.family.familyId
        from UserFamily uf
        where uf.user.userId = :userId
          and uf.family is not null
        order by uf.family.createdAt desc
    """)
    List<UUID> findLatestFamilyIdByUserId(
